...
- A first non-import graph asset compatibility runner now exists in `3DAudioGraphs-main/backend/process_graph_asset.py`, wrapping `process_audio_file` plus manifest upsert behind a real `graph/process_asset` command and returning `promotedAsset` for the newly published revision.
...
- The shared backend-call response path can now surface an explicit `promotedAsset` or `nextAsset`, and Electron now prefers that explicit revision identity when publishing revision-ready or revision-failed lifecycle updates, allowing same-asset pending revision promotion without reopening stale current-revision confirmation paths.
- The first non-import manifest-writing producer is now also explicit: `graph/process_asset` runs through the shared bootstrap or persistent-service runner path, wraps `process_audio_file` plus manifest upsert, and returns `promotedAsset` for the published graph revision instead of leaving that revision identity trapped in legacy script flow.
- Electron main and preload now also expose that producer as `graph:process-asset`, resolving a safe audio path from the request or current canonical asset and publishing `asset/revision_ready` from the returned `promotedAsset` when the originating asset revision is still current.
- The first renderer caller now also exists in the Electron `Mode Audio` control surface as `Process Current Asset`, paired with an explicit `Regenerate MFCC` toggle that now persists as a sticky user preference across asset switches and sessions before invoking `graph:process-asset` and applying the promoted revision through the existing local-integration asset path.
...
1. Expand the new renderer smoke coverage for `Mode Audio -> Regenerate MFCC / Process Current Asset` into an interactive Electron click-through, then teach the remaining real revision-producing backend jobs and onset-edit pipelines to emit manifest-backed `promotedAsset` or explicit next-revision identity through the same lifecycle contract so `revision_ready` can promote newly-derived revisions instead of only confirming the current active revision.
